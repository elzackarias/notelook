...
var images = ['images/base.jpg'];

for (var i = 0; i < arr.length; i++){
  //console.log(arr[i])
  if (arr[i] == " "){
    images.push('images/espacio.png');
  }else if(regxs.upper.test(arr[i]) == 1){
    images.push('images/'+arr[i]+'.png');
  }else{
    images.push('images/'+arr[i]+arr[i]+'.png');
  }
}
 ...
 Promise.all(jimps).then(function(data){
   ...
   for (var i = 1, coords = 210, vertical = 302; i < letras; i++, coords += 52){
     ...
     if(images[i] == 'images/yy.png'){
      data[0].composite(data[i],coords,vertical+28);
     }else if(images[i] == 'images/jj.png'){
      data[0].composite(data[i],coords,vertical+23);
     }else if(images[i] == 'images/gg.png'){
      data[0].composite(data[i],coords,vertical+20);
     }else if(images[i] == 'images/pp.png'){
      data[0].composite(data[i],coords,vertical+20);
     }else if(images[i] == 'images/qq.png'){
      data[0].composite(data[i],coords,vertical+20);
     }else{
